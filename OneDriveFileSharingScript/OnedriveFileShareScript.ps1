...
$tenantID = "paste TenantID here"
$appID = "paste clientID here"
...
function Invoke-GraphApiRequest {
    ...
    try { $result = Invoke-WebRequest -Headers $AuthHeader -Uri $uri -Verbose:$VerbosePreference -ErrorAction Stop }
        ...
        elseif ($errResp.error.code -match "Unauthorized") {  Write-Verbose "Access token has expired, trying to renew..."
                Renew-Token
...
foreach ($user in $GraphUsers) {
    ...
    Write-Output "$count Retrieving Data for user : $user"
...
# Capture the output
$global:varODFBSharedItems = $Output | select OneDriveOwner,Name,ItemType,Shared,ExternallyShared,Permissions,ItemPath | Where-Object { $_.Shared -eq "Yes" }

# Get the directory of the script
$scriptDirectory = Split-Path -Parent -Path $MyInvocation.MyCommand.Definition

# Capture the output
$global:varODFBSharedItems = $Output | Select-Object OneDriveOwner, Name, ItemType, Shared, ExternallyShared, Permissions, ItemPath | Where-Object { $_.Shared -eq "Yes" }

# Check if the variable has data
if ($global:varODFBSharedItems) {
    # File path
    $filePath = Join-Path -Path $scriptDirectory -ChildPath "$((Get-Date).ToString('yyyy-MM-dd_HH-mm-ss'))_ODFBSharedItems.csv"

    # Export the output to a CSV file
    $global:varODFBSharedItems | Export-Csv -Path $filePath -NoTypeInformation -Encoding UTF8 -UseCulture

    # Confirm the file creation
    if (Test-Path $filePath) {
        Write-Output "CSV file created successfully: $filePath"
    } else {
        Write-Output "Failed to create CSV file: $filePath"
    }
} else {
    Write-Output "No data found to export."
}

# Optionally, return the output if needed
